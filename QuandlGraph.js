...
class QuandlGraph extends React.Component {
  ...
  componentWillReceiveProps (nextProps) {
    if (!_.isEqual(this.props.api, nextProps.api)) {
      fetch(
        `https://www.quandl.com/api/v1/datasets/${nextProps.api.code}.json?rows=${nextProps.api.rows}&auth_token=3XJzrsSsLc8rzfxvwhFM`
      )
      .then((response) => {
          if (response.status >= 400) {
              throw new Error(`Bad response from server`)
    ...
    if (this.state) {
      return (
        <div className='quandle-graph'>
          <EditModal data={this.state.data} settingsDidUpdate={this.props.settingsDidUpdate}/>
